        ...
        while (nfinished < numPro) {
            ...
            for (int i = 0; i < numPro; i++) {
                if (!finished[i]) {
                    ...
                    if (possible) {
                        pw.println("Process " + i + " can be executed as need ( " + Arrays.toString(processes[i].need)
                                + " ) is lesser than or equal to the available ( " + Arrays.toString(tempRes)
                                + " ) resources.");
                        ...
                        pw.println("Available resources updated to " + Arrays.toString(tempRes) + " ( " + prevAvail
                                + " + " + Arrays.toString(processes[i].curAlloc) + " ) after Process " + i
                                + " completion");
                        safeSeq[nfinished] = i;
                        ...
                        pw.println("Process " + i + " cannot be executed (right now) as need ( "
                                + Arrays.toString(processes[i].need) + " ) is greater than the available ( "
                                + Arrays.toString(tempRes) + " ) resources.");
                        pw.println();
                    }
                }
            }
...
class Process extends Thread {
    ...
    public void requestResources(int[] reqRes) {
        ...
        for (int i = 0; i < temp3.length; i++) {
            if (temp3[i] < reqRes[i]) {
                System.out.println(
                        "Resouces cannot be allocated as requested resources is greater than available resources.\n");
                pw.println("Resouces cannot be allocated as requested resources ( " + Arrays.toString(reqRes)
                        + " ) is greater than available resources ( " + Arrays.toString(temp3) + " )\n");
                pw.flush();
                curAlloc = temp1;
                need = temp2;
                Ca.availRes = temp3;
